...
from lam4doc.services.handlers import generate_lam_report as service_generate_lam_report, \
    generate_indexes as service_generate_indexes, zip_files, FileInfo

logger = logging.getLogger(config.LAM_LOGGER)


def health():
    logger.debug("Accessing health endpoint")
    return {"message": "API is up!"}


def generate_lam_report(report_extension: str = DEFAULT_REPORT_TYPE) -> Response:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_folder:
            report_file_info: FileInfo = service_generate_lam_report(temp_folder, report_extension)

            logger.debug('finish generate lam report endpoint')
            return send_file(path_or_file=report_file_info.location,
                             download_name=report_file_info.file_name,
...
def generate_indexes() -> tuple:
    ...
    """
    logger.info('start generate lam indexes endpoint')
    try:
        with tempfile.TemporaryDirectory() as temp_folder:
...
def get_lam_files() -> tuple:
    ...
    """
    logger.info('start get lam files endpoint')
    try:
        with tempfile.TemporaryDirectory() as temp_folder:
            files_to_zip = list()
...
def upload_rdfs(body: dict,
                lam_properties_document: FileStorage = None,
                lam_classes_document: FileStorage = None,
